...
import (
	...
	offchainaggregator2 "github.com/smartcontractkit/libocr/gethwrappers2/offchainaggregator"
	testoffchainaggregator2 "github.com/smartcontractkit/libocr/gethwrappers2/testoffchainaggregator"
...
func setupOCR2Contracts(t *testing.T) (*bind.TransactOpts, *backends.SimulatedBackend, common.Address, *offchainaggregator2.OffchainAggregator) {
	key, err := crypto.GenerateKey()
	require.NoError(t, err, "failed to generate ethereum identity")
	owner := cltest.MustNewSimulatedBackendKeyedTransactor(t, key)
	sb := new(big.Int)
	...
	ocrContractAddress, _, ocrContract, err := offchainaggregator2.DeployOffchainAggregator(
		owner,
		b,
		linkTokenAddress, //_link common.Address,
		minAnswer,        // -2**191
...
func TestIntegration_OCR2(t *testing.T) {
	...
	for i := uint16(0); i < 4; i++ {
		...
		// we'll flood it with messages and slow things down. 5s is about how long it takes the
		// bootstrap node to come up.
		cfg.GeneralConfig.Overrides.OCR2BootstrapCheckInterval = 5 * time.Second
		// GracePeriod < ObservationTimeout
