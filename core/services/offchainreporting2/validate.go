...
func validateTimingParameters(config ValidationConfig, spec job.OffchainReporting2OracleSpec) error {
	lc := types.LocalConfig{
		BlockchainTimeout:                  config.OCR2BlockchainTimeout(time.Duration(spec.BlockchainTimeout)),
		ContractConfigConfirmations:        config.OCR2ContractConfirmations(spec.ContractConfigConfirmations),
		ContractConfigTrackerPollInterval:  config.OCR2ContractPollInterval(time.Duration(spec.ContractConfigTrackerPollInterval)),
...
func validateNonBootstrapSpec(tree *toml.Tree, config ValidationConfig, spec job.Job) error {
	...
	observationTimeout := config.OCR2ObservationTimeout(time.Duration(spec.Offchainreporting2OracleSpec.ObservationTimeout))
