...
func NewSingletonPeerWrapper(keyStore *keystore.OCR2, config NetworkingConfig, db *gorm.DB) *SingletonPeerWrapper {
...
func (p *SingletonPeerWrapper) Start() error {
	return p.StartOnce("SingletonPeerWrapper", func() (err error) {
		...
		p.pstoreWrapper, err = NewPeerstoreWrapper(p.db, p.config.OCR2P2PPeerstoreWriteInterval(), p.PeerID)
		if err != nil {
		...
		peerLogger := NewLogger(logger.Default, p.config.OCR2TraceLogging(), func(string) {})
		...
		peerConfig := ocrnetworking.PeerConfig{
			...
			EndpointConfig: ocrnetworking.EndpointConfig{
				...
				BootstrapCheckInterval:    p.config.OCR2BootstrapCheckInterval(),
			},
