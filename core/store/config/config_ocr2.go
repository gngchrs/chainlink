...
func (c generalConfig) OCR2P2PBootstrapPeers(override []string) ([]string, error) {
	if override != nil {
		return override, nil
	}
...
func (c generalConfig) OCR2BootstrapCheckInterval() time.Duration {
	return c.getWithFallback("OCR2BootstrapCheckInterval", parseDuration).(time.Duration)
}
...
func (c generalConfig) OCR2ObservationTimeout(override time.Duration) time.Duration {
	return c.getDurationWithOverride(override, "OCR2ObservationTimeout")
}
...
func (c generalConfig) OCR2BlockchainTimeout(override time.Duration) time.Duration {
	return c.getDurationWithOverride(override, "OCR2BlockchainTimeout")
}

func (c generalConfig) OCR2ContractSubscribeInterval(override time.Duration) time.Duration {
	return c.getDurationWithOverride(override, "OCR2ContractSubscribeInterval")
}

func (c generalConfig) OCR2ContractPollInterval(override time.Duration) time.Duration {
	return c.getDurationWithOverride(override, "OCR2ContractPollInterval")
}
...
func (c generalConfig) OCR2MonitoringEndpoint(override string) string {
	if override != "" {
		return override
	}
...
func (c generalConfig) OCR2TransmitterAddress(override *ethkey.EIP55Address) (ethkey.EIP55Address, error) {
	if override != nil {
		return *override, nil
	}
...
func (c generalConfig) OCR2KeyBundleID(override *models.Sha256Hash) (models.Sha256Hash, error) {
	if override != nil {
		return *override, nil
	}
	...
	if kbStr != "" {
		...
		if err != nil {
			return models.Sha256Hash{}, errors.Wrapf(ErrInvalid, "OCR2_KEY_BUNDLE_ID is an invalid sha256 hash hex string %v", err)
		}
		return kb, nil
	}
	return models.Sha256Hash{}, errors.Wrap(ErrUnset, "OCR2_KEY_BUNDLE_ID")
}
